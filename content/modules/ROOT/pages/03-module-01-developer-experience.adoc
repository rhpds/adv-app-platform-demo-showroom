...
. Open the **OpenShift web console** at {console_url} and log in as `{dev_user}` / `{password}`
...
_Our new developer is now in their cloud workspace and ready to contribute. They have been assigned their first task: add a new claims statistics API endpoint to the Parasol application. Using Quarkus dev mode, they can write code and see changes reflected immediately without manual builds or restarts. However, in their eagerness to ship, the developer takes some shortcuts — using `System.out.println` instead of proper logging and wrapping code in an empty catch block — code smells that will have consequences in Module 2._
...
"Our new developer has their workspace ready and has been assigned their first task: add a claims statistics API endpoint. This endpoint will aggregate claim data by category and status for the operations dashboard. Let me show you how fast this inner loop is with Quarkus dev mode. The developer writes code, saves, and the application updates immediately, no manual builds, no restarts, no waiting."

**What I do:**

. In the Dev Spaces workspace, start the application in development mode using the terminal:
+
...
. Create the new claims statistics endpoint. Add a new file `src/main/java/com/parasol/ClaimStatsResource.java`:
+
...
A: "Quarkus dev mode focuses on fast feedback for functional changes: does the code compile, does it run, does it produce the right output. Code quality enforcement happens in the CI/CD pipeline, where SonarQube applies organizational coding standards. This separation keeps the inner loop fast while the outer loop enforces quality."
...
. Switch to the **OpenShift web console** at {console_url} and log in as `{pe_user}` / `{password}` (platform engineer persona)
