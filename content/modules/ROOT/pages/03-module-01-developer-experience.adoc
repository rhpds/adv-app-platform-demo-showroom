...
+
// TODO: Add screenshot of the running Parasol app dashboard opened from DevSpaces
...
. Create the new claims statistics endpoint. In the Explorer on the left, right-click on the `java / com / Parasol` directory and select **New File**. Name the file `ClaimStatsResource.java`. Then copy and paste the following code into the new file:
+
...
* Show the available, customizable categoiies of pre-vetted services that developers and platform engineers can use when working directly in OpenShift. Platform engineers can customize this for their environment. For example, the platform engineer can set up a shared database service that developers can connect to from their applications without needing to configure credentials or network access, or remove access to certin services that are not approved for use in development environments.
+
