...
**Platform capabilities demonstrated:** RHDH catalog, RHDH templating, RHDH self-service, RHDH template lifecycle, Developer Lightspeed, Dependency Analytics, ACS, TAS (signing and admission control), Tekton Chains, TPA
...
* **Developer Hub** (RHDH) provides a single pane of glass: centralized catalog, golden path templates, template lifecycle merge requests, pipeline status, deployment status, and container images
