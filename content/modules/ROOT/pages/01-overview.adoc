...
. **Platform operations** — With the application deployed, the platform handles cross-cutting concerns: mTLS encryption and traffic management through Service Mesh, real-time observability through the Service Mesh (Kiali) and OpenShift consoles, and additional platform capabilities.

**Platform capabilities demonstrated:** MTA, DevSpaces, Quarkus, Tekton, Sonarqube, Argo CD, Service Mesh, Observability (Kiali/OpenShift console), DevSpaces code assistance (AI)
