...
. Open **GitLab** at {gitlab_url}[{gitlab_url}^] (log in as `{pe_user}` / `{password}` — platform engineer persona) and navigate to the **Argo CD production configuration repository**:
...
. Switch to **Developer Hub** at {rhdh_url}[{rhdh_url}^] and navigate to the component's **Argo CD** tab (or deployment view):
...
. Open **Developer Hub** at {rhdh_url}[{rhdh_url}^]
