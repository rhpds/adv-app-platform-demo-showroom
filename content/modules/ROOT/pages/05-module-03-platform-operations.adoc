...
* Point out the health indicators (green for healthy)
* Show the mTLS lock icons indicating encryption is active
...
**Optional: generate traffic for the graph**

[NOTE]
====
**Presenter tip:** The traffic graph looks best when there is active traffic flowing between services. Before showing the graph, start a traffic generator loop on your laptop to produce steady requests. Run this in a terminal and leave it running in the background during the demo.
...
// TODO: Add screenshot of Observe -> Metrics showing CPU and memory for Parasol workloads
image::ocp-observe-metrics.png[OpenShift Observe Metrics view showing CPU and memory utilization metrics for Parasol Insurance workloads,link=self,window=blank,align="center",width=700,title="Workload metrics: CPU and memory utilization"]

// TODO: Add screenshot of Observe -> Dashboards showing aggregate resource dashboard
image::ocp-observe-dashboards.png[OpenShift Observe Dashboards view showing aggregate CPU and memory and network metrics with customizable dashboard panels,link=self,window=blank,align="center",width=700,title="Customizable dashboards for aggregate resource monitoring"]
