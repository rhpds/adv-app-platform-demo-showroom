...
[TIP]
====
**Optional: generate traffic for the graph.** The traffic graph looks best when there is active traffic flowing between services. Before showing the graph, start a traffic generator loop on your laptop to produce steady requests. Run this in a terminal and leave it running in the background during the demo (you can also run it in the OpenShift Web Terminal but that means the traffic will originate from with the cluster, making a slightly less interesting traffic view).
...
----
while true; do
  curl -s -o /dev/null -L https://parasol-insurance-{parasol_prod_ns}.{subdomain}/dashboard.html
  curl -s -o /dev/null -L https://parasol-insurance-{parasol_prod_ns}.{subdomain}/api/claims
  sleep 5
done
----
...
[source,bat,role="copypaste",subs="attributes+"]
----
:loop
curl -s -o NUL -L https://parasol-insurance-{parasol_prod_ns}.{subdomain}/dashboard.html
curl -s -o NUL -L https://parasol-insurance-{parasol_prod_ns}.{subdomain}/api/claims
timeout /t 5 >nul
goto loop
----

Stop the loop with kbd:[CTRL+C] (or kbd:[CMD+C] on Mac) when you are done with the Service Mesh demo.
====
...
* Click on `parasol-insurance-hpa` to show the configuration: min 2, max 5, CPU target 20%
...
image::ocp-hpa-config.png[OpenShift console showing the HorizontalPodAutoscaler details with min 2 and max 5 pods and 20 percent CPU target,link=self,window=blank,align="center",width=700,title="HorizontalPodAutoscaler configured through GitOps"]

. Generate load to trigger autoscaling by running a load generator pod:
+
[source,bash,role="copypaste",subs="attributes+"]
----
oc run load-generator --image=quay.io/curl/curl:latest -n {parasol_prod_ns} --restart=Never -- /bin/sh -c "while true; do curl -s -o /dev/null http://parasol-insurance.{parasol_prod_ns}.svc:8080/api/claims; done"
----
+
...
* The blue donut grows as new replicas are added
...
// TODO: Add screenshot of topology showing scaled pods
image::ocp-hpa-scaled.png[OpenShift Topology view showing the Parasol application scaled up to multiple pods with the HPA active,link=self,window=blank,align="center",width=700,title="Autoscaler responding to load: pods scaling up automatically"]
...
* The HPA was configured through the console UI in seconds -- no YAML, no code changes
