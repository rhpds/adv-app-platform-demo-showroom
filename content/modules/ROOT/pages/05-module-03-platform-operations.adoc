...
. In the **Service Mesh** menu, navigate to **Traffic Graph** and select the Parasol namespace:
...
. **Module 2: CI/CD pipeline** — Automated pipeline triggering, SAST/Sonarqube quality gate enforcement catching code smells, and GitOps-driven delivery with Argo CD
...
A new developer joined Parasol, clicked one button to open the app in DevSpaces, added a claims statistics endpoint in seconds with Quarkus dev mode, pushed code that triggered an automated pipeline, fixed code smells flagged by Sonarqube, and deployed through GitOps, all on a platform that handles encryption, traffic management, and observability automatically. This is what an application platform delivers.
