...
[source,bash,role="copypaste"]
----
oc rollout restart deployment/parasol-db -n parasol-insurance-dev
oc rollout restart deployment/parasol-db -n parasol-insurance-secured-dev
oc rollout restart deployment/parasol-db -n parasol-insurance-secured-dev1
oc rollout restart deployment/parasol-db -n parasol-insurance-secured-dev2
oc rollout restart deployment/parasol-db -n parasol-insurance-prod
oc rollout restart deployment/parasol-insurance -n parasol-insurance-dev
oc rollout restart deployment/parasol-insurance-secured -n parasol-insurance-secured-dev
oc rollout restart deployment/parasol-insurance-secured -n parasol-insurance-secured-dev1
oc rollout restart deployment/parasol-insurance-secured -n parasol-insurance-secured-dev2
oc rollout restart deployment/parasol-insurance -n parasol-insurance-prod
----

NOTE: Not all namespaces may exist depending on how much of the demo has been run so far. It is safe to ignore errors for namespaces that do not exist.

Wait for all rollouts to complete before proceeding with the demo.
====
...
. Open the `rhdh-gitops` Argo CD at {argocd_url}[{argocd_url}^] and log in as `admin` (retrieve password from `rhdh-gitops-cluster` secret in the `rhdh-gitops` namespace) (explained above). It is expected that the `-secured` Argo CD applications may not be fully healthy at this point — they will resolve once the secured pipeline runs for the first time in Section 2
