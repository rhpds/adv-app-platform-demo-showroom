...
*Presenter note*: This is Section 3 (Intelligent Applications). The Parasol application is running with a keyword-based email routing system that sends ambiguous emails to "Review Required" for manual triage. The same developer from Section 2 is tasked with replacing this with LLM-powered classification. They re-open their DevSpaces workspace from the Section 2 feature branch, cherry-pick the LLM classification code, and push it through the pipeline. The result is intelligent email routing with no more manual review. Target duration: 15 minutes across 3 parts.
...
* Point out the emails marked **"REVIEW REQUIRED"** -- these are the ambiguous ones the keyword matcher could not handle
...
// TODO: Add screenshot of the Parasol Inbox showing emails with REVIEW REQUIRED badges
...
A: "Model governance is handled by the data science and platform teams. The application developer consumes a governed, approved endpoint. The same trusted software supply chain from Section 2 ensures the AI-enhanced code is scanned, signed, and attested before reaching production."
...
_The same developer from Section 2 is tasked with improving the email routing. They re-open their DevSpaces workspace from the feature branch created in Section 2. The pre-approved Kafka broker and LLM configuration is already available as environment variables (configured by the platform engineer through external secrets, as shown in Module 5). The developer cherry-picks the LLM classification code from a prepared branch, reviews the changes, and pushes through the pipeline._
...
"The same developer from Section 2 has been tasked with replacing the keyword matching with LLM-powered classification. Let me show you how they do this using the same platform patterns we have been demonstrating."

**What I do:**

. Re-open the **DevSpaces** workspace from the feature branch created in Section 2:
* Use the DevSpaces link from the Developer Hub catalog component created in Module 4
* Show the workspace loading on the correct feature branch

. Show the pre-configured environment variables in `src/main/resources/application.properties`:
* Point out the Kafka configuration with `DEV_KAFKA_PASSWORD` provided as an environment variable
...
// TODO: Add screenshot of application.properties showing LLM and Kafka env var configuration
...
. Open a new **Terminal** in DevSpaces and cherry-pick the LLM classification code:
+
[source,bash,role="copypaste"]
----
git cherry-pick origin/llm-classification
----
+
* "The LLM classification implementation is ready on a separate branch. The developer cherry-picks it into their feature branch. This brings in the new code without switching branches."

. Review the changes that were cherry-picked:
...
[NOTE]
====
**Presenter tip:** If time permits, the demoer can start Quarkus dev mode to show the LLM classification working locally before pushing. Use kbd:[CTRL+SHIFT+P] (or kbd:[CMD+SHIFT+P] on Mac) -> Tasks: Run Task -> devfile -> Start Development Mode. Open the app in a new tab and navigate to the Inbox to see emails being classified by the LLM in real time.
====
...
// TODO: Add screenshot of DevSpaces showing the EmailClassifier.java AI service interface
image::devspaces-email-classifier.png[DevSpaces IDE showing the EmailClassifier.java AI service interface with the LLM system prompt and department classification criteria,link=self,window=blank,align="center",width=700,title="LLM-powered email classification with Quarkus LangChain4j"]

**What they should notice:**

* The developer re-opened their existing DevSpaces workspace from the Section 2 feature branch
* The LLM endpoint and Kafka credentials were pre-configured by the platform engineer as environment variables
* The cherry-pick brought in the LLM code cleanly -- one new file and one modified file
...
A: "The API key is managed through the same external secrets pattern from Module 5. It is stored in Vault as `litellm-credentials` and injected at runtime. The developer never sees or commits the actual key."
...
* Show the **rich classification reasons** provided by the LLM for each email:
** "Existing customer Sarah Mitchell reporting roof damage from a storm -- classified as CLAIMS"
** "Unknown sender inquiring about homeowner's insurance options -- classified as ACQUISITION"
** "Existing customer Robert Chen asking about policy renewal -- classified as RETENTION"
...
image::parasol-inbox-before-after.png[Comparison showing the Parasol Inbox before with Review Required emails and after with all emails classified by the LLM,link=self,window=blank,align="center",width=700,title="Before and after: keyword matching vs LLM classification"]
...
A: "The LLM endpoint is configurable through Vault. Switching to a different model means updating the endpoint URL and API key in Vault. The application code does not change."
...
. **AI as a standard feature** -- The developer used the same DevSpaces environment, pipeline, and GitOps delivery to add LLM-powered email classification. No special tools or processes required.
