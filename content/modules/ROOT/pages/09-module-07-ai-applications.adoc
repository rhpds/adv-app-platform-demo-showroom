...
*Presenter note*: This is Section 3 (Intelligent Applications). The Parasol application is running with a keyword-based email routing system that sends ambiguous emails to "Review Required" for manual triage. The same developer from Section 2 is tasked with replacing this with LLM-powered classification. They re-open their DevSpaces workspace from the Section 2 feature branch (or create a new one if starting from here), cherry-pick the LLM classification code, and push it through the pipeline. The result is intelligent email routing with no more manual review. Target duration: 15 minutes across 3 parts.
...
use `git cherry-pick` to automatically update the existing code with the changes to implement the AI functionality (This could also be done by a more powerful LLM using a code assistant).
====
...
. Depending on the audience (whether they want a code walkthrough or not), review the changes that were cherry-picked:
