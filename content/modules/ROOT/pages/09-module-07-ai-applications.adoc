...
* An LLM is available as an API endpoint (in this demo, hosted on the Red Hat Demo Platform; in production, this could be Red Hat AI on-premises or any hosted model provider)
...
An LLM is already available as an API endpoint, ready to be consumed. It does not matter whether the model runs on-premises with Red Hat AI or through a hosted provider — the application platform treats it the same way. The question is: how does a developer build this integration without creating shadow AI outside of IT's guardrails?
...
A: "The model is available as an API endpoint. In this demo, it is hosted on the Red Hat Demo Platform, which uses OpenShift AI for model serving. In production, organizations can run models on-premises with Red Hat AI (which includes RHEL AI and OpenShift AI) or consume models from hosted providers. The application platform does not care where the model lives — the developer just needs an endpoint URL and an API key."

Q: "What about model governance and responsible AI?"::
A: "Model governance is handled by the data science and platform teams through whatever model serving infrastructure they use. The application developer consumes a governed, approved endpoint. The same trusted software supply chain from Section 2 ensures the AI-enhanced component is scanned, signed, and attested before reaching production."
...
. Switch back to **Red Hat Developer Hub** at {rhdh_url}[{rhdh_url}^]
...
. Switch to **GitLab** at {gitlab_url}[{gitlab_url}^] (log in as `{dev_user}` / `{password}`) to show the merge request:
...
. Show the deployment in the **OpenShift console** at {console_url}[{console_url}^]:
...
A: "That depends on the deployment model. If the organization runs the LLM on-premises with Red Hat AI, no data leaves their infrastructure and they maintain full data sovereignty. If they use a hosted provider, they need to evaluate the provider's data handling policies. The application platform supports both models — the endpoint URL is just configuration."
