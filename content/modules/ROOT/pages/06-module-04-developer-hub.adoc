...
----
Tell me about the parasol-insurance-api in the catalog, and write a sample snippet of Java code to access it
----
+
...
* Point out that the Parasol application appears as a **System** with multiple components underneath it (choose the *System* filter to be able to find it easily)
...
. Click on various components (e.g., the API) to show the component detail view:
...
* The developer only needs to specify a **branch name** (e.g., `feat1`) (keep the names short, as they will be used for fully qualified network names which have length restrictions))
...
. Click **Open In Catalog**, then click the **DevSpaces link** provided in the template output to open the workspace:
