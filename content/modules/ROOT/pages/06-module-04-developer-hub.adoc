...
[WARNING]
====
LLMs are non-deterministic — responses vary between attempts. The demo environment uses `qwen3-235b`. Before issuing prompts:

* Ensure `qwen3-235b` is selected in the model selection drop-down (upper right of the Lightspeed interface). Other models may be listed but will not produce meaningful results.
* If Lightspeed reports that an API (e.g. `parasol-insurance-api`) does not exist, try asking it to "list all APIs in the catalog" first, then re-ask your original question. This helps the model discover catalog context.
* As better models become available for the demo, these workarounds will no longer be needed.
====
