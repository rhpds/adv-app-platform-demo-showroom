...
*Presenter note*: This is the first module of Section 2 (Advanced Application Platform). Time has passed since Section 1: Parasol has adopted a platform engineering approach and implemented an Internal Developer Platform (IDP) with Red Hat Developer Hub. They have also established a more secure supply chain process. This module introduces Developer Hub as a single pane of glass, demonstrates Developer Lightspeed for AI-assisted development, explores the software catalog, and shows self-service environment provisioning through the "Parasol Insurance Secured Development" template. Target duration: 15 minutes across 3 parts.

[NOTE]
====
**Presenter note:** Section 2 picks up after time has passed. Parasol has adopted platform engineering practices and implemented an IDP with Red Hat Developer Hub (based on Backstage). They have also established a more secure software supply chain. The foundational platform from Section 1 is in place -- now the organization is ready to adopt advanced capabilities that improve discoverability, self-service, and software supply chain security.
====
...
// TODO: Add screenshot of RHDH home screen showing the left sidebar navigation
...
// TODO: Add screenshot of RHDH Lightspeed answering the parasol-insurance-api question
...
_Beyond discovery, RHDH provides self-service capabilities that allow developers to provision development environments from golden path templates. Instead of filing tickets and waiting for operations teams, developers use templates that the platform team has pre-approved. The "Parasol Insurance Secured Development" template creates a feature branch on the source repository and registers a new catalog component representing the developer's branch -- all the developer needs to provide is a branch name._

**Business challenge:**

* Provisioning development infrastructure requires tickets and wait times
...
* No standardized approach to feature branch development with dedicated environments
...
"The catalog solves discovery. Now let me show you self-service. Our developer needs a development environment to work on a new feature for the Parasol application. Traditionally, this means filing tickets and waiting days. Let me show you how that changes."
...
* "Each template includes everything a developer needs to get started. The platform team defined these golden paths, and the developer consumes them."

. Select the **Parasol Insurance Secured Development** template

. Walk through the template form:
* The developer only needs to specify a **branch name** (e.g., `feature/claims-ai`)
...
* Branch created on the source repository
* Development namespace provisioned on OpenShift
...
. Click the **DevSpaces link** provided in the template output to open the workspace:
* Show that DevSpaces opens to the correct feature branch automatically
...
// TODO: Add screenshot of the template form showing the branch name field
image::rhdh-secured-dev-template.png[Developer Hub software template form for Parasol Insurance Secured Development showing the branch name input,link=self,window=blank,align="center",width=700,title="Self-service development environment: one field to fill in"]

// TODO: Add screenshot of the provisioning results with DevSpaces link
image::rhdh-template-results.png[Developer Hub template provisioning results showing branch creation and catalog registration with DevSpaces link,link=self,window=blank,align="center",width=700,title="Development environment provisioned with one click"]
...
Our developer has a fully provisioned development environment on their feature branch, open in DevSpaces. In Module 5, we will see how they use the Dependency Analytics plugin to identify an unnecessary dependency, push code through the secure build pipeline, and manage secrets through Vault.
